...
class UpdateVoterPage extends StatefulWidget {
  @override
  _UpdateVoterPageState createState() => _UpdateVoterPageState();
}

class _UpdateVoterPageState extends State<UpdateVoterPage> {
  ...
  Future<void> _updateVoter(String authToken, String voterId) async {
    final String apiUrl =
        '${APIConstants.baseUrl}/api/admin/update/voter/$voterId';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Successfully updated voter
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Voter updated successfully'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed to update voter'),
              backgroundColor: Colors.red),
        );
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Update Voter Profile'),
          ...
          child: Form(
            ...
            child: Column(
              children: <Widget>[
                ...
                _imageField('New Voter Image', _voterImage,
                ...
                _imageField('New Aadhaar Image', _aadhaarImage,
                    () => _pickImage(ImageSource.gallery, false)),
                SizedBox(height: 20),
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      _updateVoter(authToken, _voterIdController.text);
                    }
                  },
                  child: Text('Update Voter'),
                  style: ElevatedButton.styleFrom(
                    primary: Color.fromARGB(255, 107, 105, 0), // Button color
                    onPrimary: Colors.white,
                  ),
  ...
  Future<void> _fetchVoterDetails(String authToken, String voterId) async {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Voter not found')),
      );
    }
  }
}
